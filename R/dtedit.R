...
dtedit <- function(input, output, name, thedata, thedata2,
	...
	editModal <- function(row) {
		...
		shiny::modalDialog(title = title.edit,
		                   ...
		                   HTML("<span style='float:center;'>"), strong("Mode: "), result$thedata[row,]$mode,
		                   HTML("</span></p>"),
		                   strong("From: "), ship_from,
		                   HTML("<br>"),
			...
			shinyBS::bsCollapsePanel("January Estimate", "Estimated Shipment Count and Average weight(kg)",
			                         ...
			                         tags$style(type="text/css", "label.control-label, .selectize-control.single{ display: table-cell; text-align: center; vertical-align: middle; } .form-group { display: table-row;}"),
			                          shiny::numericInput("result$thedata[row,]$jan_est", label = strong("Estimated Price per shipment: "),value = 0,
			                                             width=numeric.width),
			                         HTML("</span></p>")),
